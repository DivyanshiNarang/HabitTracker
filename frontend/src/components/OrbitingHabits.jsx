...
export default function OrbitingHabits() {
  ...
  const sunGradient = isDark
    ? "radial-gradient(circle at 30% 25%, rgba(136,184,149,0.45), rgba(74,124,89,0.28) 60%, rgba(47,82,57,0.2))"
    : "radial-gradient(circle at 30% 25%, #ffffff, #daeadd 45%, #b4d4ba 100%)";

  const sunShadow = isDark
    ? "inset 0 1px 0 rgba(255,255,255,0.35), 0 6px 24px rgba(74,124,89,0.32)"
    : "inset 0 1px 0 rgba(255,255,255,0.9), 0 8px 24px rgba(61,104,73,0.2), 0 2px 6px rgba(61,104,73,0.12)";

  const sunBorder = isDark
    ? "1px solid rgba(255,255,255,0.2)"
    : "1px solid rgba(255,255,255,0.8)";

  const sunIconColor = isDark ? "#b4d4ba" : "#2f5239";

  const haloBg = isDark
    ? "radial-gradient(circle, rgba(74,124,89,0.28), transparent 70%)"
    : "radial-gradient(circle, rgba(74,124,89,0.16), transparent 70%)";
  ...
  return (
    <div className="relative mx-auto w-[300px] h-[300px] sm:w-[400px] sm:h-[400px] lg:w-[460px] lg:h-[460px]">
      ...
      {STARS.map((s, i) => (
        <span
          key={i}
          className="absolute rounded-full bg-brand-300/70 dark:bg-brand-300/60"
          style={{
            top: s.top,
            left: s.left,
            width: s.size,
      ...
      >
        <Leaf className="w-1/2 h-1/2" strokeWidth={1.5} />
      </div>
